...
public class DriveBase extends Subsystem {
	...
	// DistancePerRevolution/PulsesPerRevolution;
	...
	public DriveBase() {
		...
		/*
		 * leftEncoder = new Encoder(RobotMap.ENCODER_LEFTA,RobotMap.ENCODER_LEFTB,true,
		 * EncodingType.k4X); rightEncoder = new
		 * Encoder(RobotMap.ENCODER_RIGHTA,RobotMap.ENCODER_RIGHTB,false,EncodingType.
		 * k4X); leftEncoder.setDistancePerPulse(DistancePerPulse);
	 ...
	 * public void driveForward() { reset(); while(getAverageEncoderPosition() <=
	 * 50.0){ drive.arcadeDrive(0.4, 0.4); // left, right
	 ...
	 * SmartDashboard.putNumber("Average Encoder Position",
	 * getAverageEncoderPosition()); } reset(); } private double
	 * getAverageEncoderPosition() { return (leftEncoder.getDistance() +
	 * rightEncoder.getDistance())/2; } public void turnRight() { reset(); arcLength
	 * = (PI/2)*wheelBase; while(leftEncoder.getDistance() <= arcLength/2) {
	 * drive.arcadeDrive(0.6, -0.6); } reset(); }
	 */
	@Override
